...
namespace LeonUI
{
    static public class BitmapProcessor
    {
        ...
        static public Bitmap RenderBGI(Bitmap ResourceImage,Size TargetSize,Rectangle CenterRectangle)
        {
            if (TargetSize.Equals(ResourceImage.Size)) return ResourceImage;//1
            else
            {
                Bitmap TargetBGI;
                //左上角块尺寸
                ...
                else if (TargetSize.Width < LTCellSize.Width + RBCellSize.Width && TargetSize.Height >= LTCellSize.Height + RBCellSize.Height)
                {
                    TargetBGI = new Bitmap(ResourceImage.Width, TargetSize.Height);
                    //垂直方向拉伸
                    using (Graphics TargetGraphics = Graphics.FromImage(TargetBGI))
                    {
                        //TargetGraphics.SmoothingMode = SmoothingMode.HighQuality;
                    ...
                    return new Bitmap(TargetBGI, TargetSize);
                }
                else if (TargetSize.Width >= LTCellSize.Width + RBCellSize.Width && TargetSize.Height < LTCellSize.Height + RBCellSize.Height)
                {
                    TargetBGI = new Bitmap(TargetSize.Width, ResourceImage.Height);
                    //水平方向拉伸
                    using (Graphics TargetGraphics = Graphics.FromImage(TargetBGI))
                    {
                        //TargetGraphics.SmoothingMode = SmoothingMode.HighQuality;
                ...
                {
                    return new Bitmap(ResourceImage, TargetSize);
                }
